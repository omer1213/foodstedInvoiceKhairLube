...
export class SalesReportService {
  ...
  getSalesReport(fromDate: string, toDate: string, getSalesReport: string = '1'): Observable<any> {
    // Set up query parameters
    const params = new HttpParams()
      .set('from_date', fromDate)
      .set('to_date', toDate)
      .set('get_salesreport', getSalesReport);
  ...
  getSalesReportByDateRange(fromDate: Date, toDate: Date, getSalesReport: string = '1'): Observable<any> {
    const fromDateStr = this.formatDate(fromDate);
    const toDateStr = this.formatDate(toDate);
    return this.getSalesReport(fromDateStr, toDateStr, getSalesReport);
  }
